...
def deploy_embedding_model(cfg_path: str, device: str, load_step: str) -> EmbeddingModel:
    ...
    pool = build_pool(cfg, hidden_dim, device, load_step, num_llm_layers)
...
def prepare_captions_and_statements(data: List[Dict]) -> tuple:
    ...
    for i, item in enumerate(data):
        ...
        caption_query = f"You are an assistant designed to generate high-quality images or videos based on user prompts. {caption_query} <|im_end|>"
        ...
        for statement in statements:
            statement = f"You are an assistant designed to generate high-quality images or videos based on user prompts. {statement} <|im_end|>"
